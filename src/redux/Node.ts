import { port, pos, stateData } from "../types";

export default class Node {
    constructor(public id: number, private state: stateData) { }

    get port(): port {
        return this.state.drawflow[this.id].port
    }
    
    get parent(): Node | null {
        const parentId = Object.keys(this.state.connections).find(key => key.split('_')[2] === this.id.toString())?.split('_')[0]
        return parentId ? new Node(parseInt(parentId), this.state) : null
    }
    ...
    clildren(portId: number) {
        return Object.keys(this.state.connections)
            ...
            .map(conn => new Node(parseInt((conn.split('_')[2])), this.state))
    }
